...
public final class ActivityUtils {
     ...
     * @param activity     从哪跳
     * @param clz          跳到哪
     * @param isFinishSelf 跳完是不是结束自己
     */
    public static void startActivity(Activity activity, @NonNull final Class<?> clz, boolean isFinishSelf) {
        startActivity(activity, clz, isFinishSelf, null);
    }
     ...
     */
    public static void startActivity(Activity activity, @NonNull final Class<?> clz, boolean isFinishSelf, Bundle bundle) {
        if (isFinishSelf) {
            activity.finish();
        }
        activity.overridePendingTransition(R.anim.slide_in_left, 0);//转场动画
    ...
    /**
     * code by wangjiong
     * 结束到指定Activity（会把指定activity任务栈上的都pop掉）
     *
     * @param clz           activity类
     * @param isIncludeSelf 是否结束该activity自己（(//例如：ActivityUtils.finishToActivity(MainActivity.class,false);//false是指目的MainActivity是否也finish)
     */
     ...
     * 启动Activity
     *
     * @param activity activity(从哪 自己不finish，只是startactivity)
     * @param clz      activity类（到哪）
     */
     ...
     *
     * @param activity      activity
     * @param isIncludeSelf 是否结束该activity自己（解释同上）
     * @param isLoadAnim    是否启动动画
     */
    public static boolean finishToActivity(@NonNull final Activity activity,
                                           final boolean isIncludeSelf,
