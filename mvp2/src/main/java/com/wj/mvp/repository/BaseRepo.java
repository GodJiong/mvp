...
import com.wj.mvp.utils.NApplication;
...
public class BaseRepo {
     ...
     */
    public static void post(String url, JSONObject jsonObject, Class<?> clazz, BaseRepoCallBack baseRepoCallBack) {
     ...
     * @param baseRepoCallBack 回调
     */
    public static void post(String url, JSONObject jsonObject, BaseRepoCallBack baseRepoCallBack) {
        if (isConnected()) {//是否有网络
            OkGo.<String>post(url)
                    .tag(CANCEL_TAG)
                    .upJson(jsonObject)//上传map的话可以map转jsonObject
     ...
     * @param baseRepoCallBack 回调
     */
    public static void get(String url,  Class<?> clazz, BaseRepoCallBack baseRepoCallBack) {
        if (isConnected()) {//是否有网络
            OkGo.<String>get(url)
                    .tag(CANCEL_TAG)
//                    .params("params1","value1")// url后面拼接的参数
                    .execute(new StringCallback() {
                        @Override
                        public void onSuccess(Response<String> response) {
                            success(response, baseRepoCallBack, clazz, true);
     ...
     */
    private static void success(Response<String> response, BaseRepoCallBack baseRepoCallBack, Class<?> clazz, boolean isClass) {
        if (200 == response.code()) {
            String body = response.body();
            if (!TextUtils.isEmpty(body)) {//非空判断
                try {
                    JSONObject jsonObject = new JSONObject(body);
                    boolean error = jsonObject.optBoolean("error");//根据自己业务取成功标识
                    if (!error) {//根据自己业务取成功标识
                        if (isClass) {
     ...
     */
    private static void error(Response<String> response, BaseRepoCallBack baseRepoCallBack) {
    ...
    public interface BaseRepoCallBack {
        /**
         * 交易成功
         */
